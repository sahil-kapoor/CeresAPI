...
import com.ceres.controller.v1.human.PatienRestrictionsController;
 ...
 */
public class PatientRestrictionControllerHealth extends AbstractDbEnvTestSuite {

    @Inject
    PatienRestrictionsController patientRestrictionsController;
    ...
    public void before() {
        ...
        patientRestrictionsController.setUriInfo(new UriRoutingContext(getFakeContainerRequest()));
    ...
    public void patientControllerHealth() {
        ...
        preferenceResponse = patientRestrictionsController.getAllRestrictions(patient.getId());
        assertEquals(Response.Status.OK, preferenceResponse.getStatusInfo());

        preferenceResponse = patientRestrictionsController.addRestriction(patient.getId(), new ArrayList<>(Arrays.<Long>asList(food.getId())));
        assertEquals(Response.Status.CREATED, preferenceResponse.getStatusInfo());

        preferenceResponse = patientRestrictionsController.removeRestriction(patient.getId(), food.getId());
